...
{
    public class BannerPictureControllerController : Controller
    {
      
            private readonly Lazy<IPictureService> pictureService;
            private readonly Lazy<ILoggerFacade<BannerPictureControllerController>> logger;

            public BannerPictureControllerController(Lazy<IPictureService> pictureService, Lazy<ILoggerFacade<BannerPictureControllerController>> logger)
            {
            ...
            public JsonResult UploadPicture()
            {
                ...
                for (int i = 0; i < picturess.Count; i++)
                {
                    ...
                    var path = Server.MapPath("~/Images/RestaurantBanners/") + picname;
                    pic.SaveAs(path);
